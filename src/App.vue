...
function captureScreen() {
  html2canvas(document.querySelector("#contenedor")).then((canvas) => {
    ...
    // Asigna la imagen en formato PNG como href
    link.href = canvas.toDataURL("image/png");
    // Asigna el nombre del archivo y extensión
    link.download = "screenshot.png";
    // Simula un clic en el enlace para descargar el archivo automáticamente
    link.click();
  });
}
...
function generarPDF() {
  ...
  doc.text(`Fecha del Sorteo: ${fecha.value}`, 10, 20);
  doc.text(`Ganador: ${numeroGanador.value}`, 10, 30);
  doc.text(`Lotería: ${loteria.value}`, 10, 40);
  ...
  const registrosFiltrados = conjunto.value.filter(
    (enlis) => enlis.estado === 1 || enlis.estado === 3
  );
  ...
  const saldoRecaudado = enlis.value.reduce(
    (total, enlis) => total + enlis.totalPagado,
    0
  );
  const saldoPendiente = conjunto.value.reduce(
    (total, conjunto) => total + conjunto.deuda,
    0
  );

  doc.text(`Saldo Recaudado: ${saldoRecaudado}`, 10, doc.autoTable.previous.finalY + 10);
  doc.text(`Saldo Pendiente: ${saldoPendiente}`, 10, doc.autoTable.previous.finalY + 20);
